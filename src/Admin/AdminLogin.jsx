...
function AdminLogin({ setIsAdminLoggedIn }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const { data } = await APIAdmin.post("/login", { username, password });

      // Save token in localStorage and attach to APIAdmin
...
function AdminLogin({ setIsAdminLoggedIn }) {
  ...
  const [password, setPassword] = useState("");
  const [adminCode, setAdminCode] = useState(""); // new admin code field
  const navigate = useNavigate();
