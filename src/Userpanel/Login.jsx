    ...
    <div className="d-flex align-items-center justify-content-center min-vh-100" 
      ...
      <div className="card p-5 shadow-lg rounded-4" style={{ minWidth: "400px" }}>
        ...
        <p className="mt-4 text-center text-muted">
          Don’t have an account? <Link to="/signup" className="text-decoration-none fw-bold text-danger">Sign Up</Link>
        </p>
      </div>
    </div>
  );
...
function Login({ setIsLoggedIn }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setIsLoggedIn(true);
      setRedirect(true); // trigger navigation after re-render
    } catch (err) {
      alert(err.response?.data?.message || "Login failed");
      console.error(err);
    }
    ...
    <div className="d-flex align-items-center justify-content-center min-vh-100"
      ...
      <div className="card p-5 shadow-lg rounded-4" style={{ minWidth: "400px" }}>
        ...
        <p className="mt-4 text-center text-muted">
          ...
          <Link to="/signup" className="text-decoration-none fw-bold text-danger">
            Sign Up
          </Link>
        </p>
      </div>
