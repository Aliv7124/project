...
function MyPosts() {
  ...
  const { socket } = useNotification(); // ✅ get socket from context
  ...
  useEffect(() => {
    ...
    const handleNewItem = (data) => {
      setPosts(prev => [data.item, ...prev]); // add new item at top
      const audio = new Audio("/notification.mp3");
      audio.play();
    };

    const handleUpdateItem = (data) => {
      setPosts(prev => prev.map(p => p._id === data.item._id ? data.item : p));
      const audio = new Audio("/notification.mp3");
      audio.play();
    };

    const handleDeleteItem = (data) => {
      setPosts(prev => prev.filter(p => p._id !== data.itemId));
      const audio = new Audio("/notification.mp3");
      audio.play();
    };
